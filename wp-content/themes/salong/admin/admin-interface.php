...
function siteoptions_add_admin() {
    ...
    $tt_page = add_theme_page( "主题设置", "主题设置", 'edit_theme_options', 'siteoptions','siteoptions_options_page');
	...
	<div class="wrap" id="truethemes_container">
		...
		<form action="" enctype="multipart/form-data" id="ofform">
    		<div id="header">
      			<div class="logo">
        			<h2>远方的雪山主题选项面板</h2>
        			<p>感谢你对<a href="http://yfdxs.com" target="_blank" title="远方的雪山">远方的雪山</a>的支持！技术支持：66895271<br>欢迎光顾<a href="http://taji.me" target="_blank" title="它季|专属民族格调商城！">它季商城</a>，<a href="http://tajime.taobao.com" target="_blank" title="它季|专属民族格调商城！">它季淘宝</a></p>
      			</div>
      			<div class="clear"></div>
    		</div>
    		<?php 
    		...
    		<input type="submit" value="保存所有选项" class="button-primary" />
  		</form>
		
 		<form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    		<span class="submit-footer-reset">
    		<input name="reset" type="submit" value="重置选项" class="button submit-button reset-button" onclick="return confirm('CAUTION: Any and all settings will be lost! Click OK to reset.');" />
...
function of_ajax_callback() {
	...
	elseif ($save_type == 'options' OR $save_type == 'salong') {
		$data = $_POST['data'];
...
function siteoptions_uploader_function($id,$std,$mod){
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">更新图片</span>';
